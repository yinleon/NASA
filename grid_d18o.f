      ...
      IMPLICIT NONE
      INTEGER,PARAMETER :: skip1=-99,nfil=25000,FILTNT=1
     *     ,ii=10000,NTTEST=102,REGCT=24,levdep=0,imWOA=360
     *     ,jmWOA=180,lmWOA=33
      REAL,PARAMETER :: skip2=-99.99,saltyNA=36.5
      CHARACTER*100 FILEIN,d18OFILE,NOTEST,REFT,phosfile,salfile
     *     ,tempfile,o2file,mldfile,salout,titfort,FILEOUT,ESTFILE
     ...
     *     ,geormsERR(regct),fortest1(1:imWOA,1:jmWOA)
...
c          if (calcd18o.lt.-5) print*,tlon,tlat,calcd18o
 ...
 945      CONTINUE
     ...
     *     /trim(clev(wdep))
      write(210) titfort,fortest(1:imWOA,1:jmWOA)
...
c      print*,"waterms: ",workm,dep,mld,pstar,phosnatl,phosmix
     ...
     *       (index(basincode(workm),'BaffinBa').gt.0).or.
...
**********************2 DISTANCE CALCULATOR*****************************
      ...
      IMPLICIT NONE
      REAL,PARAMETER :: pi=3.14159,radius=6371.
...
***********************************************************************
      ...
      integer, parameter :: im=360, jm=180, regct=24
      integer i,j,k,kt4(im,jm),workm(im,jm),as(regct,6)
      CHARACTER*80 MASKA,MASKB,MASKC
c     *     ,namebin
      character*8 basincode(regct)
     ...
     *     ,n_baffinba,n_depacind,n_intnopac,n_basin(8)
c     *     ,n_intindoc
